...
export const ffmpegCommandLive = (liveUrl: string, fileName: string): string => {
 return `ffmpeg -re -stream_loop -1 -i "${liveUrl}" -c:v libx264 -preset veryfast -maxrate 1500k -bufsize 3500k -g 60 -pix_fmt yuv420p -c:a aac -b:a 128k -f flv "${fileName}" -loglevel error > /dev/null 2>&1 &`    
